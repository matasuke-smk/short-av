    ...
    <div className="h-[100dvh] flex flex-col bg-black overflow-hidden">
      ...
      {showVideoModal && (
        ...
        >
          ...
          {/* 価格表示と詳細ページボタン */}
          <div
            className="w-full px-4 mt-4 flex items-center gap-3"
            onTouchStart={handleModalTouchStart}
            onTouchEnd={handleModalTouchEnd}
          >
            {/* いいねボタン - 詳細リンクボタンの左側に配置 */}
            {currentVideo && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="flex-shrink-0 bg-black/70 backdrop-blur-sm rounded-full p-4 transition-all active:scale-90 hover:bg-black/90 shadow-lg"
                aria-label="いいね"
              >
