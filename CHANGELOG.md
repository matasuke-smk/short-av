...
#### PC表示を横レイアウトに改善
**目的**: PC表示を綺麗で見やすいレイアウトに改善

**変更内容**:
- メイン画面とすべてのモーダルでPC表示（lg: 1024px以上）時に横レイアウトを適用
- 左側55%: 動画表示エリア
- 右側45%: ボタン・情報エリア
- 動画モーダル: 左15%操作エリア、中央flex-1動画、右側バナー
- 動画プレイヤーのサイズを最大化: `h-[90vh]`で固定し、画面の90%を使用

**対象コンポーネント**:
1. `VideoSwiper.tsx` - メイン動画表示
2. `SearchModal.tsx` - 検索モーダル
3. `RankingModal.tsx` - ランキングモーダル
4. `LikedModal.tsx` - いいね済みモーダル
5. `HistoryModal.tsx` - 履歴モーダル

**レイアウト比較**:
- **モバイル縦**: 動画上、ボタン下（変更なし）
- **モバイル横**: 左動画、右ボタン（変更なし）
- **PC**: 左動画、右ボタン（新規追加） ← スマホ横と同じ

**技術実装**:
- `landscape:` クラスに `lg:` クラスを追加
- レスポンシブ対応を強化（縦・横・PC全てで最適化）

**効果**:
- PC表示が直感的で見やすいレイアウトに
- 動画とボタンが同時に見える使いやすい配置
- スマホ横表示と同じ統一されたUX

**コミット**:
- `30470f6` - 初回実装
- `1ee80ec` - サムネイルの左寄せ配置を修正
- `740be0e` - 動画モーダルをPC画面でも横レイアウトで表示
- `d91716d` - 動画モーダルのレイアウト改善（動画を大きく、バナーを高さ最大に）
- `0459b9a` - 動画を幅いっぱいに大きく、バナーを上下中央配置に改善
- `65ef144` - 動画プレイヤーのサイズを最大化（h-[90vh]で固定）
- `8364875` - アスペクト比制約を解除して動画を90vhで表示
- `dbaeb52` - 動画プレイヤーの幅をw-fullに修正して大きく表示

---

### Bug Fixes（バグ修正）

#### robots.txtで不要なページのクロールをブロック
**問題**: 「クロール済み - インデックス未登録」が113件発生

**原因**:
- テストページ（/test-db, /test-dmm）がクロールされている
- 管理ページ（/admin/size-stats）がクロールされている
- ユーザー固有ページ（/history, /liked）がクロールされている
- これらにはnoindex設定済みだが、robots.txtでブロックされていなかった

**修正内容**:
- `app/robots.ts`に以下を追加:
  - `/test-*` - テストページを除外
  - `/admin/` - 管理ページを除外
...
**効果**:
- Googlebotが不要なページをクロールしなくなる
- 「クロール済み - インデックス未登録」が大幅に減少する見込み
- クロールバジェットを重要なページ（記事ページ等）に集中できる
